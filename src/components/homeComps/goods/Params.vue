<template>
  <div class="all">
    ...
    <el-card>
      ...
      <!-- tab 页签区域 -->
      <el-tabs v-model="activeName" @tab-click="handleTabClick" ref="tab">
        <!-- 添加动态参数的面板 -->
...
import { getFromByIdandPa } from 'network/goods/getFromByIdandPara'
import { addParams } from 'network/goods/addParams'
import { getParamsById } from 'network/goods/getParamsById'
import { editParams } from 'network/goods/editParams'
import { deletedParams } from 'network/goods/deleteParams'
...
export default {
  ...
  methods:{
    ...
    getParamsData(){
      // 根据所选分类的id和对应的面板获取参数
      getFromByIdandPa(this.cateId,this.activeName).then(res => {
        
        if(res.meta.status !== 200){
          return this.$message.error('获取父级分类数据失败！请选择分类！')
        }else{
    ...
    showEditDialog(id){
      ...
      // 查询当前参数的信息
      getParamsById(this.cateId,id,this.activeName).then(res => {
          console.log(res);
          if(res.meta.status !== 200){
    ...
    showInput(row){
      ...
      // 简单来说：按钮点击后还没出现文本框，所以获取不到input，要异步
      this.$nextTick(_ => {
    ...
    saveAttrVals(row){
      // 需要发起请求保存这次操作
      editParams(this.cateId,row.attr_id,row.attr_name,
          row.attr_sel,row.attr_vals.join(' ')).then(res => {
        console.log(res);
        if(res.meta.status !== 200){
    ...
    removeParams(id){
      this.$confirm('此操作将永久删除该参数, 是否继续?', '提示', {
        ...
        deletedParams(this.cateId,id).then( res => {
          console.log(res);
          if(res.meta.status !== 200){
  ...
  computed: {
    ...
    isBtnDisabled(){
      return this.selectedKeys.length === 0 ? true : false
    },
    // 这是当前选中的分类Id
    cateId(){
